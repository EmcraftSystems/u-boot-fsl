...
/* clang-format off */
static const clock_name_t source_mcuxpresso[][8] = {
...
{
    return source_mcuxpresso[root][src];
}
...
static inline uint32_t CLOCK_GetRootClockFreq(clock_root_t root)
{
    ...
    freq = CLOCK_GetFreq(source_mcuxpresso[root][mux]) / (CLOCK_GetRootClockDiv(root));
    assert(freq);
    return freq;
}
