...
static int promate_send_data(struct spi_slave *spi, u8 value)
{
	u16 data[1] = { value | (1 << 8) };
	int err = spi_xfer(spi, 9, data, NULL, SPI_XFER_BEGIN | SPI_XFER_END);
...
static int promate_send_command(struct spi_slave *spi, u8 cmd, int args_cnt, ...)
{
	u16 data[1] = { cmd };
	int err = spi_xfer(spi, 9, data, NULL, SPI_XFER_BEGIN | SPI_XFER_END);
